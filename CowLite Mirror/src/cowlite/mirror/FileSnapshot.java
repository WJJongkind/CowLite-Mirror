 ...
 * method has been called and for comparison to other directories by using the {@link #compareTo(FileSnapshot, List, List)}
 * method. 
 * 
 * <p>
 * 
 ...
 * the main functionality of {@code FileSnapshot} remains monitoring files and directories
 * and their subdirectories, to see if files have been added, altered or deleted. 
 * 
 * <p>
 * 
 * {@code FileSnapshot}s can also be stored to a {@code File} so that they can be
 * used for comparison at a later time, either when the stored {@code FileSnapshot} becomes
 ...
 * implemented to decompile the stored {@code FileSnapshot}s. However, this functionality
 * will be implemented soon.
 * 
 * <h1>Basic usage</h1>
 * {@code FileSnapshot} can be used to monitor directories for changes. In the example
 * below, the C:\ disk on a Windows machine is monitored. In this example, we first initialize
 * the {@code FileSnapshot} object and then immediatly update it without recording
 * any changes:
 * 
 * <pre><i>
 ...
 * </i></pre>
 * 
 * The {@code FileSnapshot} can also be compared to another {@code FileSnapshot}.
 * If there is another disk, D:\, then we can compare the difference in files between
 * C:\ and D:\. In the example below we compare the {@code FileSnapshot} of above
 * to a new {@code FileSnapshot}:
 * 
 * <pre><i>
...
{
    /**
     * Obtain one single FileSystemProvider for all {@code FileSnapshot}s. This should
    ...
    /**
     * The time at which the directory which is represented by the {@code FileSnapshot}
    ...
    /**
     * The size of the directory or file that is represented by the {@code FileSnapshot},
     * as recorded by the last {@link #update(List, List, List)} call.
     */
    private long size;
    
    /**
     * {@code true} if the {@code FileSnapshot} represents a directory, {@code false}
     * if the {@code FileSnapshot} represents a file.
     */
    private boolean directory;
    
    /**
     * All subdirectories of the directory that is referenced by the {@code FileSnapshot} represented
     * as a {@code Map}. 
     */
     ...
     * @param p The {@code Path} object of which a {@code FileSnapshot} should be created.
    ...
    /**
     * Updates the {@code FileSnapshot} and all subdirectories so that their metadata
     ...
     * by the {@code FileSnapshot} then these directories are added to the monitoring-structure
     * as well.
     * <p>
     ...
     *                {@code FileSnapshot}s to the list of which the monitored
     ...
     *              {@code FileSnapshot}s to the list of files/directories that
    ...
    /**
     * This method is called when the {@code FileSnapshot} is monitoring a directory.
     * All subdirectories and files of which {@code FileSnapshot}s exist are updated.
     * For subdirectories and files of which no {@code FileSnapshot}s exist, new
     * {@code FileSnapshot}s are created and immediatly updated.
     * 
     * @param deleted The list in which deletion events are recorded by adding 
     *                {@code FileSnapshot}s to the list of which the monitored
     *                directory or file has been removed. If this parameter is null,
    ...
    /**
     * Stores the {@code FileSnapshot} and all {@code FileSnapshot}s that represent
     * subdirectories and their files to the specified {@code File}. The data is compiled
     * by writing the data of a single {@code FileSnapshot} to each line of the file. Each
     ...
     * @throws FileNotFoundException When the file to which the {@code FileSnapshot}
     ...
     * @return A {@code FileSnapshot} which contains all subdirectories that were
    ...
    /**
     * Returns the {@code Path} object which is being monitored by the {@code FileSnapshot} object
     * that this method is being called on. 
     * @return The {@code Path} object which is being monitored by the {@code FileSnapshot} object
     * that this method is being called on. 
     */
    public Path getFile() {
        return file;
    ...
    /**
     * Returns {@code true} if the {@code FileSnapshot} is monitoring a directory.
     * @return {@code true} if the {@code FileSnapshot} is monitoring a directory.
     */
     ...
     * that is being monitored by the {@code FileSnapshot} object.
    ...
    /**
     * Returns the time at which the file that is represented by the {@code FileSnapshot}
     * was created in milliseconds.
     * @return The time at which the file that is represented by the {@code FileSnapshot}
     * was created in milliseconds.
     */
    ...
    /**
     * Returns the children of the {@code FileSnapshot}, which are subdirectories and
     * files contained by the directory that the {@code FileSnapshot} represents.
     * @return The children of the {@code FileSnapshot}, which are subdirectories and
     * files contained by the directory that the {@code FileSnapshot} represents.
     */
