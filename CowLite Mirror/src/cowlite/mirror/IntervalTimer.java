...
/**
 * This class can be used to time the intervals at which filecheckers should be
 * activated. It does this multithreaded if requested. When too many threads are
 * being used, the filecheckers are added to a queue.
 *
...
public class IntervalTimer implements ActionListener {
    ...
    /**
     * Map containing the checkers and the amount of ticks that have been
     * registered for them. One tick equals one second.
     */
     ...
     * will use one core for each FileChecker, or the maximum amount of system
     * cores available if there are too many file checkers.
     */
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    System.out.println(new Date() + "-------" + "Something went wrong during file check...");
                    e.printStackTrace();
                    THREADS.release();
                }
            }
